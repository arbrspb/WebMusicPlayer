// librosa_stats.js 14-08-25 01-50
document.addEventListener('DOMContentLoaded', function() {
    // Получаем путь к папке из data-атрибута
    let folder = document.getElementById('genreStatsRoot').dataset.folder;

    // Автоматически добавляем текущую папку в разрешённые (на сервере)
if (folder) {
    fetch('/add_temp_music_root', {
        method: 'POST',
    ...
    .then(data => {
        if (data.success) {
            console.log('Папка добавлена во временно разрешённые:', folder);
        } else {
            console.warn('Не удалось добавить разрешённую папку:', data.error);
        }
    });
}
    // Экспорт JSON
